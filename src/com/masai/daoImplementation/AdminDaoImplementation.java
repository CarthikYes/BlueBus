...
public class AdminDaoImplementation implements AdminDao {
	...
	public String delete_Bus(int bus_no) {
		...
		try (Connection connection = DatabaseUtility.provideConnection()) {
			...
			PreparedStatement statement = connection.prepareStatement("DELETE FROM BUS WHERE BUS_NO = ?");
	...
	public String create_Bus_Route(Bus_Route bus_route) {
		...
		try (Connection connection = DatabaseUtility.provideConnection()) {
			...
			statement.setString(4, bus_route.getDestination());
	...
	public String delete_Route(int route_id) {
		...
		try (Connection connection = DatabaseUtility.provideConnection()) {
			...
			PreparedStatement statement = connection.prepareStatement("DELETE FROM ROUTE WHERE ROUTE_ID = ?");
